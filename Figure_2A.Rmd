...
plot <- plot_plasticityIndex(df = amine_data, xvar = strain, BayesFit = TRUE, width = 0.2, alpha = 0.7, bar = TRUE) + labs(title = "day corrected") +
  scale_color_plot(palette = "grey-blue", drop = TRUE) + guides(color = FALSE) +
  scale_fill_plot(palette = "grey-blue", drop = TRUE) + guides(fill = FALSE) +
  facet_grid(.~genotype+strain) + coord_cartesian(ylim = c(-1.5,3.5)) +
