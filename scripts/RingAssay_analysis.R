...
ring <- read_csv(here::here("data","Fruc_ring.csv")) %>%
  mutate(food = fct_relevel(food, "OP50"),
         plate = factor(seq(1:nrow(.))),
         fraction_out =  Nout/Ntotal)
...
ring %>%
  ...
  ggbeeswarm::geom_quasirandom(aes(color = date)) +
...
#### glycerol data ####
  glycerol <- read_csv(here::here('extdata/Figure_S1a.csv'))
    mutate(food = fct_relevel(food, "OP50"))
